...
namespace MonoTouch.Dialog
{
	...
	public abstract class BaseBooleanImageElement : BoolElement {
		...
		public class TextWithImageCellView : UITableViewCell {
			...
			static UIFont font = UIFont.SystemFontOfSize (fontSize);
	...
	public class StyledStringElement : StringElement {
		...
		public UIView ActivityView
		{
			get { return cell.AccessoryView; }
			set { cell.AccessoryView = value; }
		}
	...
	{
		static NSString pkey = new NSString ("PickerElement");
		static UIToolbar toolBar;
		static UIPickerView pickerView;
		static UIDatePicker datePickerView;
		static UITableViewCell activeCell;
		
		private PickerType pickerType;
		private UITableViewCell cell;
		private PickerData pickerData;
		private DateTime dateTime;
		private UIDatePickerMode dateMode;
		private UIBarButtonItem doneButton;
		private int[] selectedRows;
		
		protected internal static NSDateFormatter fmt = new NSDateFormatter () {
			DateStyle = NSDateFormatterStyle.Short
		};
		
		public event Action<int[]> RowsSelected;
		public event Action Dismissed;
		
		/// <summary>
		/// Gets or sets the string used to determine how to lay out the selections of the picker.
		/// For example, "{0} + {1} != {2}" with the selections 11, 12, and 24 would yield "11 + 12 != 24".
		/// </summary>
		public string FormatString
		{
			...
			set
			{
				// null format strings are not supported
				if (value != null)
				{
					formatString = value;
				
					//force the value to refresh whenever the format string is changed
					FormatValue();
				}
			}
		}
		...
		/// <summary>
		/// Creates a picker with custom options in a single column.
		/// </summary>
		public PickerElement (string caption, IEnumerable<string> entries, int selected)
			: this (caption, new List<string[]>() { entries.ToArray() }, selected)
		{
		}
		...
		/// </summary>
		public PickerElement (string caption, DateTime value, UIDatePickerMode mode) : base (caption, FormatDate(value))
		{
		...
		/// <summary>
		/// Programmatically selects the rows indicated by the given integer array.
		/// The selection can optionally be animated.
		/// </summary>
		public void SelectRows (int[] indices, bool animated)
		{
			pickerView.Model = pickerData;
			selectedRows = new int[pickerData.Data.Length];
			indices.CopyTo(selectedRows, 0);
			
			for (int i = 0; i < selectedRows.Length; i++)
			{
				pickerView.Select(selectedRows[i], i, animated);
			}
		...
		public override UITableViewCell GetCell (UITableView tv)
		{	
			cell = tv.DequeueReusableCell (pkey);
			if (cell == null)
			{
				cell = new UITableViewCell (UITableViewCellStyle.Value1, pkey);
				cell.SelectionStyle = UITableViewCellSelectionStyle.Blue;
			}
		...
		public override void Selected (DialogViewController dvc, UITableView tableView, NSIndexPath path)
		{
			...
			case PickerType.Custom:
				...
				for (int i = 0; i < selectedRows.Length; i++)
					pickerView.Select(selectedRows[i], i, false);
				
				view = pickerView;
				break;
			case PickerType.Date:
				pickerView.RemoveFromSuperview();
				datePickerView.Mode = dateMode;
				datePickerView.Date = dateTime;
				view = datePickerView;
				break;
			}
			...
			toolBar.Items = new UIBarButtonItem[] { new UIBarButtonItem(UIBarButtonSystemItem.FlexibleSpace, null), doneButton };
			
			if (view.Superview == null)
				dvc.Add(view);
			
			if (toolBar.Superview == null)
				dvc.Add(toolBar);
			
			tableView.ContentSize = new SizeF(tableView.Frame.Width, tableView.Frame.Height + pickerView.Frame.Height + toolBar.Frame.Height);
			tableView.SetContentOffset(new PointF(0, cell.Frame.Y - 33), true);
		}
		...
		private void FormatValue ()
		{
			...
			for (int i = 0; i < selectedRows.Length; i++)
				strings[i] = pickerData.Data[i][selectedRows[i]];
			
			// if the format string is invalid, just add each string together
			try { Value = string.Format(formatString, strings); }
			catch (FormatException) { Value = string.Concat(strings); }
		...
		{
			return fmt.ToString(value) + " " + value.ToLocalTime ().ToShortTimeString ();
		}
		...
		public void selectIt(int item)
		{
			pickerView.SelectedRowInComponent(item);
		}
		...
		public class PickerData : UIPickerViewModel
		{
			...
			public string[][] Data
			{
				...
				set
				{
					...
					{
						float width = 48;
						// the last component gets whatever space hasn't been taken up
						...
						// the other columns get sized according to the length of the longest option
						else
						{
							foreach (string s in data[i])
							{
								float size = pickerView.StringSize(s, UIFont.BoldSystemFontOfSize(20)).Width + 20;
								if (size > width)
									width = size;
							}
						}
			...
			public PickerData(IEnumerable<string[]> data)
			{
				Data = data.ToArray();
			}
	...
	public class RadioElement : StringElement {
		...
		public override void Selected (DialogViewController dvc, UITableView tableView, NSIndexPath indexPath)
		{
			...
			if (RadioIdx != root.RadioSelected){
				...
				{
					System.Diagnostics.Debug.WriteLine("RadioElement.Selected(): " + e.Message);
				}
				finally
				{
	...
	public class EntryElement : Element {
		...
		{
			return entry == null ? false : entry.ResignFirstResponder();
		}
		
		public bool BecomeFirstResponder()
		{
			return entry == null ? false : entry.BecomeFirstResponder();
		}
		...
		/// Initial value.
		/// </param>
		public EntryElement (string caption, string placeholder, UITextAlignment alignment, string value) : base (caption)
		{
			Value = value;
		...
		/// </param>
		public EntryElement (string caption, string placeholder, UITextAlignment alignment, string value, bool isPassword) : base (caption)
		{
			Value = value;
	...
	public class DateTimeElement : StringElement {
		...
		private DateTime minimumDate = new DateTime(0);
		...
		public DateTimeElement (string caption, DateTime date, DateTime MinimumDate) : this (caption, date)
		{
