...
namespace MonoTouch.Dialog
{
	public class DialogViewController : UIViewController
	{
		...
		UITableViewStyle style = UITableViewStyle.Grouped;
		...
		// background view that will be the parent of the UITableView,
		// needed to inset the table view for a graphic surronding it
		private BackgroundView backgroundView;
		
		public NinePatchImage Background9Image { get; set; }
		...
		public override void ViewDidLoad ()
		{
			...
			NSNotificationCenter.DefaultCenter.AddObserver ( "UIKeyboardDidShowNotification", (notification) => { 
				...
				foreach (var sec in this.Root.Sections)
				{
					foreach (var item in sec) 
					{  
						var mle = item as MultiLineEntryElement;
						
						if (mle != null && mle.IsResponder())
						{
							var c = this.TableView.CellAt(mle.IndexPath);
							System.Diagnostics.Debug.WriteLine("Selected " + mle.Caption + ": bottom " + c.Frame.Y.ToString());
							offSet = (int)c.Frame.Y;
						}
						
						var e = item as EntryElement;
						
						if (e != null && e.IsResponder())
						{
							var c = this.TableView.CellAt(e.IndexPath);
							System.Diagnostics.Debug.WriteLine("Selected " + e.Caption + ": bottom " + c.Frame.Y.ToString());
							offSet = (int)c.Frame.Y;
						}
					}
				}

				RectangleF rec = UIKeyboard.BoundsFromNotification(notification);
				RectangleF frame = View.Frame;
				
				//frame.Y -= (offSet - rec.Height);
				if(offSet < rec.Height)
				{
					frame.Y -= offSet;
					//frame.Y -= (rec.Height - offSet) ;
				}				
				else
				{
					frame.Y -= (rec.Height) ;
			...
			NSNotificationCenter.DefaultCenter.AddObserver ( "UIKeyboardWillHideNotification", (notification) => { 
				UIView.BeginAnimations(string.Empty, System.IntPtr.Zero);
				UIView.SetAnimationDuration(0.3);
		...
		{			
			...
			NSNotificationCenter.DefaultCenter.RemoveObserver(this, "UIKeyboardDidShowNotification", null);
			//var note = NSNotificationCenter.DefaultCenter;
		...
		{
			System.Diagnostics.Debug.WriteLine("LayoutSubViews: " + backgroundView.Frame.ToString());
			
			if (Background9Image != null)
			{
				RectangleF tableFrame = Background9Image.GetContentRectangleForSize(backgroundView.Frame.Size);
		...
		private class BackgroundView: UIView
		{
			...
			public BackgroundView(DialogViewController dialogController)
			{
